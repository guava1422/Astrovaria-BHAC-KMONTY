...
#            self.pointdata.Update()
    ...
    def getCellData(self):
        self.getData()
        if self.data.GetCellData().GetNumberOfArrays() == 0:
            p2c = v.vtkPointDataToCellData()
            # vtk 4,5:
#            p2c.SetInput(self.data)
            # vtk 6:
            p2c.SetInputData(self.data)
            celldata=p2c.GetOutput()
            # vtk 4,5:          
#            self.celldata.Update()
            # vtk 6:
            p2c.Update()
            nvars=celldata.GetCellData().GetNumberOfArrays()
            for i in range(nvars):
                varname = celldata.GetCellData().GetArrayName(i)
                self.data.GetCellData().AddArray(celldata.GetCellData().GetArray(varname))
    ...
    def getVars(self):
        ...
        #also load the point data into varname_p    
        nvars= self.data.GetPointData().GetNumberOfArrays()
        for i in range(nvars):
            varname = self.data.GetPointData().GetArrayName(i)
            if (self.silent == 0): print("Assigning point-variable: "+varname+"_p")
            vtk_values = self.data.GetPointData().GetArray(varname)
            exec("self.%s_p = ah.vtk2array(vtk_values)[0:self.npoints]" % (varname))
...
class particles():
    ...
    def __init__(self,offset,file='data',components=3,filetype='snapshot'):
        self.offset=offset
        self.filenameout = file
        self.isLoaded = False
    ...
    def makefilename(self):
        ...
        elif self.filetype == 'ensemble':
            self.filename = self.filenameout+'_ensemble'+str(self.offset).zfill(6)+'.dat'
        else:
            self.filename = self.filenameout+'_destroyed'+'.dat'
...
class ensemble():
    ...
    def __init__(self,offset,file='data0000',npayload=1,components=3,delimiter=',',filetype='ensemble'):
        self.offset=offset
        self.filenameout = file
        self.isLoaded = False
        self.components = components
    ...
    def makefilename(self):
        if self.filetype == 'ensemble':
            self.filename = self.filenameout+'_ensemble'+str(self.offset).zfill(6)+'.csv'
        else:
            self.filename = self.filenameout+'_destroyed'+'.csv'            
...
class cartesian():
    ...
    '''interpolate vtu pointdata (vtuBmpi) to ordinary (inclined) caresian mesh'''

    def __init__(self,offset,file='data',npixel=[64,64,64],L=[10,10,10],phi=0,theta=0,delta=0,x0=[0,0,0],type='vtu'):
    ...
    def make_grid(self):
        ...
        xi,yi,zi=np.mgrid[0:self.npixel[0],0:self.npixel[1],0:self.npixel[2]]

        xi=xi/(self.npixel[0]-1.)
        yi=yi/(self.npixel[1]-1.)
        zi=zi/(self.npixel[2]-1.)

        xi = xi*self.L[0] - self.L[0]/2.
        yi = yi*self.L[1] - self.L[1]/2.
        zi = zi*self.L[2] - self.L[2]/2.
    ...
    # Now rotate the points around theta and then phi:
        ...
        costheta = np.cos(self.theta*np.pi/180.)
        sintheta = np.sin(self.theta*np.pi/180.)
    ...
    # Optional: rotate around delta to orient view:
        if self.delta==0:
            return [xidd+self.x0[0],yidd+self.x0[1],zidd+self.x0[2]]
        else:
            cosa = np.cos(self.delta*np.pi/180.)
            sina = np.sin(self.delta*np.pi/180.)
            # make sure to use the original phi angle: 
            n1 = sintheta*np.cos(self.phi*np.pi/180.)
            n2 = sintheta*np.sin(self.phi*np.pi/180.)
            n3 = costheta
    ...
    def get_pointdata(self):
        ...
        self.datareader.SetFileName(self.filename)
        self.datareader.Update()
        self.pointdata = self.datareader.GetOutput()

        vtk_points=self.pointdata.GetPoints().GetData()
    ...
    def get_var(self,varname):
        try:
           ...
           var = self.extract(varname)
           var_grid = self.ontoGrid(var)
           setattr(self,varname,var_grid)
        return getattr(self,varname)

    def extract(self,varname):
        '''Extracts variable "varname" from vtk datastructure "data"'''

        if self.pointdata.GetPointData().GetNumberOfArrays() > 0:
            vtk_values = self.pointdata.GetPointData().GetArray(varname)
        else:
            print('no pointdata in vtu file')

        return ah.vtk2array(vtk_values)

    def ontoGrid(self,var):

        print ('=== Obtaining variable on uniform mesh ===')

        x=self.points[:,0]
        y=self.points[:,1]
        z=self.points[:,2]

        data = griddata((x,y,z),var,(self.grid[0],self.grid[1],self.grid[2]),method='nearest')

        return data.reshape(self.npixel)

    def getVarnames(self):
        nvars= self.pointdata.GetPointData().GetNumberOfArrays()
        self.varnames=[]
        for i in range(nvars):
            self.varnames.append(self.pointdata.GetPointData().GetArrayName(i))
